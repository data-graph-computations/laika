...
is called \defn{chromatic scheduling}.  


In chromatic scheduling one
...
%\vspace{-2ex}
  ...
  \begin{minipage}[t]{.6\linewidth}
      \begin{codebox*}
        \Procname{$\proc{JonesPlassmann}(G)$}
        \li let $G=(V,E,\rho)$
        \li \Parfor $v \in V$ \Do
    ...
    \proc{JonesPlassmann} uses
      a recursive helper function \proc{JP-Update} to process a vertex
      using the user-supplied \proc{Update} function once all of its
      predecessors have been updated, recursively calling \proc{JP-Update}
      on any successors who are eligible to be updated.  The function
      \proc{Join} decrements its argument and returns the post-decrement
      value.}
...
scheduling \proc{JonesPlassmann} described in 
\figref{jones_plassmann} involves initializing a counter at each vertex with the 
...
corresponding to physical simulations in \secref{laika}.
...
as we explore in \secref{reordering}.  
...
curve is given in \figref{2d_hilbert}.  The red curve is the first 
...
\secref{reordering}.  In \secref{laika} we will describe
a new scheduling algorithm \proc{Laika} which exploits this
cache advantage and test its performance in \secref{empirical}.
We will explore the theoretical and experimental
properties of an extension to \proc{Laika} that enables
distributed memory operation in \secref{partitions}.
